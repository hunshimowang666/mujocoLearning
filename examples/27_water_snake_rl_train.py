"""Train the water-snake path controller with mjlab GPU-parallel simulation."""
...
class WaterSnakeMjlabTrainConfig:
...
def direct_train_config() -> WaterSnakeMjlabTrainConfig:
  return WaterSnakeMjlabTrainConfig()
...
def train(args: WaterSnakeMjlabTrainConfig) -> None:
  ...
  env_cfg = water_snake_path_env_cfg()
  ...
  if resume:
    ...
    if args.resume_latest:
      ...
      resume_path = get_checkpoint_path(
        PROJECT_ROOT / "logs" / "rsl_rl" / agent_cfg.experiment_name,
        agent_cfg.load_run,
        agent_cfg.load_checkpoint,
      )
    print(
      "[INFO] Resuming water snake mjlab training from "
...
def main() -> None:
  ...
  if len(sys.argv) == 1:
    ...
    print("[INFO] Running 27_water_snake_rl_train.py with direct-run settings")
  else:
    args = tyro.cli(WaterSnakeMjlabTrainConfig)
  train(args)
