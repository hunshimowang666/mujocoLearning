"""
20_go2_joint_table_rl_train.py
==============================
Train an RL torque controller that tracks a table of Unitree Go2 joint targets.

Target table:
  examples/go2_joint_target_angles.csv
  One row per target moment, one column per joint, values in degrees.

Usage:
  ./venv/bin/python examples/20_go2_joint_table_rl_train.py
  ./venv/bin/python examples/20_go2_joint_table_rl_train.py --episodes 3000
  ./venv/bin/python examples/20_go2_joint_table_rl_train.py --timesteps 1000000
  ./venv/bin/python examples/20_go2_joint_table_rl_train.py --resume
"""
...
LOG_DIR = os.path.join(SCRIPT_DIR, "logs", "go2_joint_table")
TARGET_TABLE = os.path.join(SCRIPT_DIR, "go2_joint_target_angles.csv")
...
class EvalCallbackWithVecNormalize(EvalCallback):
    def _on_step(self) -> bool:
        ...
        if self.best_mean_reward > previous_best and self.best_model_save_path is not None:
            ...
            print(f"[Go2JointTableRL] Saved best VecNormalize to {norm_path}")
        return continue_training


def make_env():
    return Go2JointTableEnv(
...
def delete_old_outputs():
    for path in (MODEL_DIR, LOG_DIR):
        if os.path.exists(path):
            shutil.rmtree(path)
            print(f"[Go2JointTableRL] Deleted old output: {path}")


def get_checkpoint_norm_path(model_path):
    if model_path.endswith("go2_joint_table_latest.zip"):
...
def find_latest_resume_model():
    candidates = glob.glob(os.path.join(MODEL_DIR, "go2_joint_table_latest.zip"))
    candidates += glob.glob(os.path.join(MODEL_DIR, "go2_joint_table_ppo_*_steps.zip"))
    if not candidates:
        return None
...
def train(timesteps, resume, delete_old_networks):
    if resume:
        print("[Go2JointTableRL] Resume is enabled; old network deletion is skipped.")
    ...
    latest_model = os.path.join(MODEL_DIR, "go2_joint_table_latest")
    ...
    if resume_model is not None:
        ...
        print(f"[Go2JointTableRL] Resuming from {resume_model}")
    ...
    checkpoint_cb = CheckpointCallback(
        ...
        save_path=MODEL_DIR,
        name_prefix="go2_joint_table_ppo",
    ...
    env.close()
    eval_env.close()
    print(f"[Go2JointTableRL] Saved model to {latest_model}.zip")
    print(f"[Go2JointTableRL] Saved VecNormalize to {latest_norm_path}")
...
def main():
    ...
    print(
        f"[Go2JointTableRL] episodes={args.episodes}, max_episode_steps={MAX_EPISODE_STEPS}, "
        f"num_envs={NUM_ENVS}, device={DEVICE}, total_timesteps={timesteps}"
    )
    print(
        f"[Go2JointTableRL] target_table={TARGET_TABLE}, "
        f"target_row_duration={TARGET_ROW_DURATION:.3f}s, "
        f"interpolate_targets={INTERPOLATE_TARGETS}"
    )
    print(
        f"[Go2JointTableRL] resume={args.resume}, delete_old_networks={delete_old_networks}, "
